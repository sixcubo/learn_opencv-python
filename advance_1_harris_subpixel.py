...
# Harris特征可参考视频： https://www.bilibili.com/video/av64112410/?p=11
...
"""
# 通常意义上来说，角点就是极值点，即在某方面属性特别突出的点。

def harris_demo(image):
    """
    利用角点的一个特性：向任何方向移动变化都很大。将窗口向各个方向移动（u，v）然后计算所有差异的总和。
    ...
    # 函数原型：cornerHarris(src, blockSize, ksize, k, dst=None, borderType=None)
    # @param src：输入图像，必须为float32
    # @param blockSize：角点检测中要考虑的领域的大小
    # @param ksize：Sobel求导中使用的窗口大小
    # @param k：Harris 角点检测方程中的自由参数，取值范围基本为为[0.04, 0.06]
    dst = cv.cornerHarris(gray, 2, 3, 0.04)

    dst = cv.dilate(dst, None)  # 角点膨胀，使其突出
    print(dst.max())
    # 取出膨胀后dst.max()的1%以上角点,标记为红色
